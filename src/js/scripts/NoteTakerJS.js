export default class NoteTakerJS {
  async getJS() {
    //  VARIABLES
    ...
    function creaNota() {
      ...
      newArticle.className =
        'flex flex-col flex-wrap grid-article block p-6 rounded-lg shadow-lg bg-white max-w-sm hover:cursor-pointer'
      newArticle.id = title.innerText
      btnDel.innerText = 'Borrar'
      btnDel.id = 'btnDel'
      btnDel.className =
        'self-end px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out'

      modal.className =
        'modal fixed flex jsutify-center top-0 left-0 hidden bg-gray-700 bg-opacity-50 w-full h-full outline-none overflow-x-hidden overflow-y-auto px-6 py-20'
      modalDialog.className =
        'modal-dialog mx-auto max-w-xl pointer-events-none'
